...
db = mysql.connector.connect(host="localhost", user="root", password = "annajavi")
mycursor = db.cursor()
mycursor.execute("CREATE DATABASE IF NOT EXISTS sarabase")
db.close()
db2 = mysql.connector.connect(host="localhost", user = "root", password = "annajavi", database = "sarabase")
...
if not check_if_exists("do yoga"):
    s = date.today()
    intervall2 = timedelta(days=1)
    yesterday = s - intervall2
    create_habit("do yoga", 7, 1, 7, 14, yesterday, 3)
    
if not check_if_exists("bake a cake"):
    s = date.today()
    intervall2 = timedelta(days=2)
    l_c_d = s - intervall2
    create_habit("bake a cake", 1, 4, 4, 4, l_c_d, 0)

if not check_if_exists("get out of bed"):
    s = date.today()
    intervall2 = timedelta(days=1)
    l_c_d = s - intervall2
    create_habit("get out of bed", 7, 1, 9, 12, l_c_d, 2)

if not check_if_exists("read a book"):
    s = date.today()
    intervall2 = timedelta(days=8)
    l_c_d = s - intervall2
    create_habit("read a book", 1, 0, 0, 1, l_c_d, 2)
...
while menu_choice != "quit":
    print ("Hello, this is Sara's habit tracker!")
    ...
    if menu_choice == "new":
        new_habit = input("Please name your new habit ")
        gate = True
        while gate == True:
            try:
                periodicity = int(input("How often would you like to repeat your new habit per week? Write a number from 1 (once a week) to 7 (daily) "))
                gate = False
            except ValueError:
                gate == True
    ...
    # when 'see' is chosen the user can either see all their habits or only those of a chosen periodicity 
        ...
        see_choice = input("If you would like to see all your habits write 'all', if you would only like to see those habits of a specific periodicity write 'specific'. ")
        if see_choice == "all":
            for i in object_list:
                print("task:{name}, periodicity: {periodicity}, begin date: {begindate}, current run streak: {crs}, longest run streak: {lrs}, number of fails: {failure}".format(name=i.name, periodicity=i.periodicity, begindate=i.begin_date, crs = i.current_streak, lrs = i.longest_streak, failure=i.fails)) 
            ...
            while gate == True:
                try:
                    chosen_periodicity = int(input("For which periodicity would you like to see all your habits? "))
                    gate = False
                except ValueError:
                    gate == True
                    print("Please write a number. Try again!")
    ...
    # 'quit' exits the while loop  
        ...
        gate = True
        while gate == True:
            checkedoff_task = input("Which task would you like to check off? ")
            for i in object_list:
                if checkedoff_task == i.name:
                    gate = False
            if gate == True:
                print("You do not have a task of that name, try again.")
        ...
        print("Thank you, your task was checked off. Well done!")
        ...
        if ppp == 7:
            ...
            if fff == 1 and current_s%7 == 0:
                ...
                print ("Congratulations! You have managed to complete this task for {y} week(s). Keep it up!".format(y=weeks))
        else:
            if ppp == fff:
                weeks = ccc + 1
                if weeks%4 == 0:
    ...
    # if a user wants to see the analysis of their habits they can either see their longest streaks or their most failures
        ...
        analyse_choice = input("If you want to see your longest run streak write 'longest streak', if you want to see with which task you are struggling most write 'failure'. ")
        if analyse_choice == "longest streak":
            streak_choice = input("If you like to see the longest run streak of all daily tasks write 'all daily', if you like to see the longest run streak of all weekly tasks write 'all weekly', or if you want to see the longest run streak of a specific task write 'specific'. ")
            if streak_choice == "specific":
                for i in object_list:
                    print(i.name)
                gate = True
                while gate == True:
                    next_choice=input("For which task would you like to see your longest streak? ")
                    for i in object_list:
        ...
        print ("You have to choose between 'see', 'new', 'check off', 'delete', 'analyse', and 'quit'. Try again!")
